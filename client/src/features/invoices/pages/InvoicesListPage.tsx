...
import { useInvoicesQuery } from "../api/useInvoices";
...
export function InvoicesListPage() {
  ...
  const { data, isLoading, isFetching } = useInvoicesQuery({ page, pageSize, search: debouncedSearch || undefined });
    ...
    <Stack spacing={2.5}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ justifyContent: "space-between", alignItems: { sm: "center" } }}>
        ...
        {hasPermission("Invoices", "create") && (
          <Button variant="contained" startIcon={<FiPlus />} onClick={() => navigate("/app/invoices/new")}>
            New Invoice
          </Button>
        )}
      </Stack>
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  key={row.id}
                  hover
                  onClick={() => navigate(`/app/invoices/${row.original.id}`)}
