...
const schema = z.object({
  ...
  paymentMethod: z.enum(["Cash", "UPI", "Bank", "Card", "Credit"]),
...
export function InvoiceFormPage() {
  ...
  const totals = useMemo(() => {
    ...
    for (const item of watchedItems ?? []) {
      ...
      // Mirrors InvoiceService.CreateAsync: a GST-disabled company bills at zero tax
      // regardless of each product's own rate — the preview must match what gets saved.
      if (isGstEnabled) {
  ...
  const onSubmit = async (values: FormOutput) => {
    ...
    try {
      ...
      setServerError(getErrorMessage(error, "Unable to create the invoice."));
    }
  };
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 4 }}>
                <Controller name="invoiceDate" control={control} render={({ field }) => (
                  <TextField {...field} label="Invoice date" type="date" slotProps={{ inputLabel: { shrink: true } }} fullWidth />
                )} />
              </Grid>
              <Grid size={{ xs: 12, sm: 4 }}>
                <Controller name="paymentMethod" control={control} render={({ field }) => (
                  <TextField {...field} select label="Payment method" fullWidth>
                    {["Cash", "UPI", "Bank", "Card", "Credit"].map((m) => <MenuItem key={m} value={m}>{m}</MenuItem>)}
                  </TextField>
                )} />
              </Grid>
            </Grid>
            ...
            <Grid container spacing={2}>
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <Controller name="paidAmount" control={control} render={({ field }) => (
                  <TextField {...field} label="Amount paid now" type="number" helperText="Leave 0 if this is a credit sale" fullWidth />
                )} />
              </Grid>
            </Grid>

            <Stack direction="row" spacing={1.5} sx={{ justifyContent: "flex-end" }}>
              <Button onClick={() => navigate("/app/invoices")}>Cancel</Button>
