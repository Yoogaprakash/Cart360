...
import { useSalesReturnsQuery } from "../api/useReturns";
...
export function SalesReturnsListPage() {
  ...
  const { data, isLoading, isFetching } = useSalesReturnsQuery({ page, pageSize });
  ...
  return (
    <Stack spacing={2.5}>
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} sx={{ justifyContent: "space-between", alignItems: { sm: "center" } }}>
        <Typography variant="h5" sx={{ fontWeight: 800 }}>Sales Returns</Typography>
        {hasPermission("SalesReturns", "create") && (
          <Button variant="contained" startIcon={<FiPlus />} onClick={() => navigate("/app/sales-returns/new")}>New Sales Return</Button>
        )}
      </Stack>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={columns.length} align="center" sx={{ py: 6, color: "text.secondary" }}>No sales returns found.</TableCell></TableRow>
            ) : (
